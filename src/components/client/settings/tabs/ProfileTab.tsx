      ...
      onSubmit={async (values) => {
        try {
          ...
          if (selectedImage) {
            ...
            updatedProfile.avatar = imageUrl; // Update the avatar URL in the profile
          }
      ...
      {({ values, setFieldValue, dirty }) => (
        <Form className="space-y-6">
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex flex-col items-center space-y-4">
              <Avatar className="w-24 h-24 border-4 border-white dark:border-gray-700 shadow-md">
                <AvatarImage src={selectedImage ? URL.createObjectURL(selectedImage) : values.avatar} alt={values.name} />
                <AvatarFallback className="text-3xl">
                  {values.name.charAt(0)}
                </AvatarFallback>
              </Avatar>
              <input
                ...
                onChange={(event) => {
                  if (event.target.files && event.target.files[0]) {
                    ...
                    setFieldValue("avatar", URL.createObjectURL(file)); // Update Formik field for preview
                    setShowUpdateButton(true);
                  }
                }}
              />
